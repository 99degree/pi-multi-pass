...
import { getModels, StringEnum, type Api, type Model } from "@earendil-works/pi-ai";
...
async function searchWeb(query: string, maxResults = 5): Promise<SearchResult[]> {
	const response = await fetch(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`, {
		headers: {
			"User-Agent": "Mozilla/5.0 pi-multi-pass search tool",
		},
	});
	if (!response.ok) {
...
export default function multiSub(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"Use search when current web information is needed before answering.",
		],
